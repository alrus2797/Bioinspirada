...
ga_utils.show_map_table('Poblacion Inicial', population, np.array(range(params.n_population)).reshape(-1,1),lambda x, y: print(ga_utils.show_route(x)))
apptitutes = list(map(lambda x: (ga_utils.get_route_cost(routes, x[1]), x[0]), enumerate(population)))
...
for iteration in range(params.iterations):
	...
	ga_utils.show_map_table('Poblacion Actual - Aptitudes', population, np.array(apptitutes)[:,0],lambda x, y: print(ga_utils.show_route(x),' => ',y))
	...
	for i, (m1, m2) in enumerate(parents):
		...
		print(f'Padres: {m1} - {m2} => {idp1} - {idp2} => {ga_utils.show_route(population[idp1])}, {ga_utils.show_route(population[idp2])}')
		print('Cruzamiento? ', end='')
		if np.random.random() < params.p_cross:
			print('Si')
			child1, child2 = ga_utils.obx_cross(population[idp1],population[idp2])
		else: print('No')
		new_population.append(child1)
		new_population.append(child2)
		print("Hijos: ")
		print(f'{ga_utils.show_route(child1)} - {ga_utils.show_route(child2)}')
		# Mutation starts

		print('Mutacion? Hijo 1 ', end='')
		if np.random.random() < params.p_mut:
			print('Si')
			ga_utils.simple_mut(child1)
			print('Nueva forma:', ga_utils.show_route(child1))
		else: print('No')

		print('Mutacion? Hijo 2 ', end='')
		if np.random.random() < params.p_mut:
			print('Si')
			ga_utils.simple_mut(child2)
			print('Nueva forma:', ga_utils.show_route(child2))
		else: print('No')
		print()
...
ga_utils.show_map_table('Poblacion Final', population, np.array(apptitutes)[:,0],lambda x, y: print(ga_utils.show_route(x),' => ',y))
print('Best:', ga_utils.show_route(best_of_best), '=>',best_curr_gen[0], 'Iteration', best_it)
